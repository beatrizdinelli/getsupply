# Consuming GetSupply Design System in web apps

Read `artifacts/getsupply-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/getsupply-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/getsupply-ds/styles.css";
```
...
`node_modules/@workspace/getsupply-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/getsupply-ds/components/ui/button";
import { cn } from "@workspace/getsupply-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/getsupply-ds/hooks/use-toast";
```
...
`@workspace/getsupply-ds/components/ui/button`. Run the app's typecheck
