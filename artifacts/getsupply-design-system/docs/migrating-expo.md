# Migrating Expo UI to GetSupply Design System

Read `artifacts/getsupply-ds/docs/AGENTS.md` and
`artifacts/getsupply-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/getsupply-ds/lib/native-theme` and
  `@workspace/getsupply-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/getsupply-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/getsupply-ds` and retains only
